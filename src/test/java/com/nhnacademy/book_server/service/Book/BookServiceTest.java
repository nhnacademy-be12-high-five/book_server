...
//            ReflectionTestUtils.setField(book, "id", 100L); // ID가 생성된 것처럼 시뮬레이션
//            return book;
//        });
//
//        // [추가] EntityManager는 void 메서드이므로 별도 Stubbing은 필요 없으나,
//        // 호출 시 아무 동작도 하지 않도록 Mock이 주입되어 있어야 합니다.
//
//        // when
//        Book createdBook = bookService.createBook(dto);
//
//        // then
//        assertNotNull(createdBook); // 이제 null이 아님
...
//
////        // (선택) flush나 refresh가 호출되었는지 검증하고 싶다면 추가
////        verify(em).flush();
